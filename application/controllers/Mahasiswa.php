...
class Mahasiswa extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('MahasiswaWali_model');
	...
	public function mahasiswaView()
	{
		$username = $this->session->userdata('username');
		if (!empty($username)) {
			$data = array('isi' => 'pages/Wali/page_mahasiswa', 'title' => 'Mahasiswa');
			$this->load->view('layout/wrapper', $data);
		} else {
			$this->load->view('pages/page_login');
		}
	...
	{
		$list = $this->MahasiswaWali_model->get_datatables(); //getAllData
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $field) {
			$no++;
		...
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->MahasiswaWali_model->count_all(),
			"recordsFiltered" => $this->MahasiswaWali_model->count_filtered(),
			"data" => $data,
		);
		//JSON output
		echo json_encode($output);
